...
const puppeteer = require('puppeteer-core')
  ...
  ; (async () => {
    ...
    // V2.0 Task 4：onboarding 后 applyAvatar 会换随机形象（hiyori→haru 等），
    // 换载窗口期 meS.model 为 null（旧模型销毁→新模型加载），必须重新等模型就绪
    const waitModel = async () => {
      for (let i = 0; i < 30; i++) {
        const has = await page.evaluate(() => !!(window.__pixi && window.__pixi.meS && window.__pixi.meS.model && window.__pixi.meS.model.visible))
        if (has) return true
        await new Promise((r) => setTimeout(r, 500))
      }
      return false
    }
    if (!(await waitModel())) { console.log('MODEL_GONE_AFTER_ONBOARD'); await browser.close(); return }

    const info = await page.evaluate(() => {
      const s = window.__pixi.meS
      const b = s.realBounds() // 角色真实绘制范围（getBounds 是 moc 画布矩形，含大块空白）
      if (!b) return JSON.stringify({ err: 'realBounds null' })
    ...
    const { err, bx, by, bw, bh } = JSON.parse(info)
    if (err) { console.log('FATAL_INFO_' + err); await browser.close(); return }
    ...
    // ---- 单击互动（poke）----
    const rb = await page.evaluate(() => { const b = window.__pixi.meS.realBounds(); return b ? { x: b.x, y: b.y, w: b.width, h: b.height } : null })
    if (!rb) { console.log('FATAL_realBounds_null_after_drag'); await browser.close(); return }
    const cxCur = Math.round(rb.x + rb.w / 2)
    const cyCur = Math.round(rb.y + rb.h * 0.55)
