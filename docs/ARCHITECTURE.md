...
### 8.3 Loop Engineering 修复（V1.2.1，2026-09-04）
- **单渲染树原则（关键）**：canvas 必须常驻。此前引导页/主界面两个 `return` 各建一棵树，React 换树时 `.canvas-host` 节点被重建，append 进去的 Pixi canvas 随旧节点丢弃 → 所有已建身份用户看不到分身（线上实测 canvas=0）。修复后 canvas-host 是 `.space` 第一个子节点，其余全部条件渲染
- **触屏适配**：拖拽增量改 `clientX/Y` 差值（`movementX` 在移动端是 undefined → NaN 把模型算飞）；长按 550ms 弹菜单（此前触屏无菜单入口）；canvas `touch-action:none`（防浏览器把手势判成滚动后发 pointercancel）；viewport `user-scalable=no`；拖拽同步 `target/home` 消除与 tick 回位 lerp 的互拉
- **对方模型懒加载**：`partner` 状态就绪才 `partnerS.load()`（未绑定用户省约 1MB 首屏流量）；加载完成按 `stateRef` 恢复可见性/表情，规避 getPartner 与模型加载的竞态
- 排查结论存档：gzip 生效（879KB→251KB）、Cubism Core 经 Vite 重写路径正常加载；此前的"未压缩/404"均为测量方式误判
...
## 9. 换装系统（V1.3，2026-09-04）

> 需求：不同的穿搭形象风格，可切换。设计原则：**零新增美术资产**——形象用已有双模型，穿搭用色彩滤镜。

### 9.1 形象切换（模型级）
- 可选形象：Hiyori · 活泼 / Natori · 沉稳；新建身份时 `/api/identity` 随机分配（50/50）
- `AvatarSprite.swap()`（`client/src/live2d/avatar.ts`）：解绑 `beforeModelUpdate` → destroy 旧模型 → 清心情插值/Worker/Blob URL → 加载新 model3；App 层在 swap 完成后恢复 `home` 位置、重新 `bindDrag`、重放心情表情
- 模型表 `MODELS` 与标签 `AVATAR_LABELS` 常量驱动，加新形象只需各加一行

### 9.2 穿搭风格（V1.3.1 肤色安全重构）
- ~~V1.3.0 用 ColorMatrixFilter 全模型滤镜~~ → **被否决：脸和肤色会一起变色**
- 新方案：**模型像素零改动**——
  - **配饰**：emoji `PIXI.Text`（🎀🧢🌻👑🧣），`head/side/neck` 三种挂点按模型渲染宽高定位，每帧同步位置 + 轻微摇摆/呼吸；换模型（swap）后自动重建
  - **光环**：canvas 生成的白色径向渐变纹理（模块级共享），`tint` 上色置于 stage 最底层（模型身后），alpha 0.5±0.06 呼吸脉冲
  - `OUTFIT_STYLES`：`{label, emoji, aura, anchor, swatch}`；风格 ID 与 V1.3.0 一致，服务端存量数据无缝兼容
- 时序安全：模型未加载时仅记录 style，`load()/swap()` 完成后自动应用；`swatch` 同时用作 UI 芯片 active 描边色

### 9.3 数据与同步
- **持久化**：`users` 加 `style TEXT DEFAULT 'default'`（幂等迁移 try/catch）；`avatar/style` 存 users 行 + 客户端 `da_me` localStorage 双写
- **API**：`POST /api/state` 扩展 `avatar/style` 可选参数，更新后随响应回传 `{state, avatar, style}`；`GET /api/state/:id` 同样返回
- **实时同步**：`state_update` socket 事件携带 `avatar/style`，对端收到后 `swapMyModel()/applyStyle()` 即时换装，无需刷新
