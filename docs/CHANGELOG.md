...
### 1. 主 JS 分包（首屏瘦身）
- 原 `App.tsx` `git mv` 为 `AppStage.tsx`（保留文件历史），新 [App.tsx](../client/src/App.tsx) 为薄引导层：`React.lazy` 按需加载 AppStage/Admin，Suspense 显示 BootFallback；动态 chunk 拉取失败 12s 后出现「重新加载」按钮（attempt state 重建 lazy 节点，真实重新发起 import，非 location.reload）
...
- 新增 [application/outbox.ts](../client/src/application/outbox.ts)（纯函数内核 + localStorage 适配，key `da_outbox_v1`）：eventId 入队幂等、上限 50 条淘汰最旧、flush 并发复用同一 promise、顺序回放、服务端 duplicate 视为送达出队、失败计数保留、maxTries=20 死信丢弃、坏 JSON 安全降级空队列；12 项单测
- [sendInteraction.ts](../client/src/application/sendInteraction.ts)：`SendInteractionResult` 携带 `eventId/payload`；REST 彻底失败时 AppStage 据此入队并 toast「已暂存，联网后自动送达」
- 触发回放：socket `connect`、window `online`、`visibilitychange` 回前台、顶栏「📶 N 条消息等待联网送达」按钮手动重试；送达后刷新时间线
- 契约验证 [scripts/verify-offline-replay.mjs](../scripts/verify-offline-replay.mjs)：真实启动 express（内存 SQLite），5 项——离线期间零落库、重连顺序回放且同 eventId 合并、二次回放 duplicate 出队、新事件不被旧队列污染、坏载荷不阻塞好条目
...
- saver 保底档 20→15fps；策略抽为纯函数 [live2d/perf-policy.ts](../client/src/live2d/perf-policy.ts)（TIER_FPS/DOWNGRADE_RATIO=0.75/WINDOW_MS=4000/downgradeTier/resolveStartTier），perf.ts 仅保留 RAF 采样与应用；暴露 pausedInBackground 供自动化断言；10 项单测
- [public/sw.js](../client/public/sw.js) 缓存版本升 `da-cache-v2.0.0`：导航网络优先、断网回退应用壳；`/assets/*.js|css|woff2` stale-while-revalidate；models/live2d 资产仍 cache-first；API/Socket 不干预

### 5. Jing/Tao 发布门禁
- [scripts/verify-avatar-contract.mjs](../scripts/verify-avatar-contract.mjs)：`evaluateRelease()`（placeholder 或契约校验失败即不可发布，异常收敛为结果不抛出）；node:test 4 项（占位阻断/合法放行/删 moc3 阻断/缺目录）；`pnpm verify:contract --release` 全绿才退出 0。当前 jing/tao 占位包被正确阻断——**正式资产接入前不得发布**
...
### 1. 根因取证（moc 部件清单）
- [chitose.cdi3.json](../client/public/models/chitose/chitose.cdi3.json) 部件只有「体/两腕/头发/五官」，**没有「下半身」部件**（对照组 natori 有「下半身」）——Chitose 官方素材本身就是大腿以上半身模型，腿在 moc 画布底边被截断，任何定位都无法让腿出现
- 次因：各模型原生画布尺寸差异极大（chitose 天然高约 2500 单位），固定 `MODEL_SCALE=0.12` 时 chitose 只渲染 ~300px（视口 850 的 35%），又小又"缺腿"

### 2. 修复方案（[avatar.ts](../client/src/live2d/avatar.ts) + [App.tsx](../client/src/App.tsx) + [models.ts](../client/src/live2d/models.ts)）
- **身高归一（"高富帅"）**：加载后按实测 `naturalH = model.height / scale` 直接换算绝对 scale——全身模型统一渲染 **74% 视口高**（原 35% → 翻倍），半身像模型渲染 **100% 视口高**；测量失败保底原比例
- **半身像构图**：`AvatarDef.halfBody` 标记（chitose），home y 按"胸像立绘"规则 `innerHeight - h/2 + 0.1h`——截断边压出屏幕底 10%，画面只露头到腰，构图自然不再有"缺腿"感
- **全身像锚定**：home y = `innerHeight - 96 - h/2`，脚底贴 Dock 上沿（按实测半高动态计算，替代上一版拍脑袋的 300px）
- **拖拽防沉底**：拖拽 clamp 按"脚底不出屏"取 `innerHeight - h/2`，高模型拖到底也不缺腿
- **形象切换重算 home**：swapMyModel / swapPartnerLook 成功与回滚路径都按新模型 halfBody 规则重算 y（旧 home 是上一个模型的构图）

### 3. 验证
- 本地：verify-invite 9/9 + verify-v160 13/13 双绿；目检截图（412×850）：chitose modelH=850/y=510/截断边出屏 85px，胸像构图完整挺拔
- 生产：部署 bundle index-CSHu-bWa.js（备份 dist.bak.20260906-182759），线上双 E2E 全绿（见 ACCEPTANCE D10）

## [V1.6.1] 2026-09-06（master）—— 缺腿修复 + 桌宠对标 Codex + 邀请链接分享闭环

> 用户反馈："温柔青年很帅，但是缺腿了，补上；做成 codex 桌宠类似的，上网搜索借鉴优秀的代码；测试发给别人的时候能不能接收到链接，同步状态；loop engineering"。详见 [versions 副本](./versions/20260906_【TS】_CHANGELOG_V1.6.1.md)。

### 1. 缺腿修复（全形象受益）
- **根因**：模型 home y=`innerHeight*0.78`（663px），模型半高 150 → 脚部 813px，被互动 Dock（~730）和底栏（~794）双重遮挡，全身形象膝盖以下不可见
- **修复**：home y 改为 `max(0.5h, innerHeight-300)`——模型半高 150 + 底部 UI 预留 150，脚部落在 Dock 之上；自己与对方分身同规则（[App.tsx](../client/src/App.tsx)）
- 截图验证：Chitose 西装+格纹裤全身完整（shots/legs-companion.png 修复前后对比）

### 2. 桌宠模式对标 Codex（调研证据驱动）
- **调研**（2026-09-06）：[CodexPetDesk](https://github.com/fangbm/CodexPetDesk)（透明置顶+状态行 idle/running/failed）、[Copiwaifu](https://github.com/Panzer-Jack/Copiwaifu)（Live2D 桌宠状态机六态映射动作组）、[live2Dpet_Miku](https://github.com/wasd09090030/live2Dpet_Miku)（Electron 桌宠：每 12s 自动待机动画防"摆大字挂机"）
- **落地三点**：
  - `AvatarSprite.startIdleLoop/stopIdleLoop`：空闲 12s 随机轮换 Idle 动作组（多动作模型不再摆大字；页面隐藏时跳过配合渲染暂停）；进入桌宠启动/退出停止
  - 桌宠状态行 `.pet-status`：Dock 上方迷你胶囊显示「💫 TA 名字 · 状态/在线」（对标 Codex Pet Desk 状态显示）
  - 桌宠背景修复：原 `background: transparent` 露出白色 body（网页无系统级透明窗），恢复深色渐变沉浸背景

### 3. 邀请链接分享闭环 E2E（[verify-invite.cjs](../client/verify-invite.cjs) 9/9 全绿）
- T1 链接格式（origin+BASE_URL+?invite=code）/ T2 复制反馈 / **T3 全新用户打开链接→onboard→自动接受→双端互见** / T4 状态同步（A设开心→B可见）/ T5 穿搭同步（B换海盐蓝→A.partnerS.style=ocean）/ T6 无效码提示 / T7 已绑定打开新邀请 already_bound 守卫 / T8 桌宠状态行
- 邀请链路本就完整（老用户 URL 直接受、新用户 `__pendingInvite` onboard 后补接受），本轮以 E2E 固化防回归

### 4. 验证
- 本地：verify-invite 9/9 + verify-v160 回归 13/13 双绿
- 生产：部署 bundle index-DDLctPxF.js（备份 dist.bak.20260906-174141），线上双 E2E 全绿
